...
const ARBITER_ADDRESS = "0xTuWalletDeArbitrajeAqui";
...
export default function AgreementScreen() {
  ...
  const handleSign = async () => {
    ...
    try {
      // ── DEBUG — revisá estos valores en la consola del browser ──
      console.log("=== DEBUG AGREEMENT ===");
      console.log("address (cliente):", JSON.stringify(address));
      console.log("freelancer.wallet raw:", JSON.stringify(freelancer.wallet));
      console.log("ARBITER_ADDRESS:", JSON.stringify(ARBITER_ADDRESS));
      console.log("budget:", JSON.stringify(budget));
      console.log("deadlineDays:", JSON.stringify(deadlineDays));

      const freelancerWallet = freelancer.wallet.trim().replace(/[^\x20-\x7E]/g, "");
      const clientAddress = address!.trim().replace(/[^\x20-\x7E]/g, "");

      console.log("freelancerWallet sanitizada:", JSON.stringify(freelancerWallet));
      console.log("clientAddress sanitizada:", JSON.stringify(clientAddress));
      console.log("freelancer válida:", ethers.isAddress(freelancerWallet));
      console.log("cliente válida:", ethers.isAddress(clientAddress));
      console.log("árbitro válido:", ethers.isAddress(ARBITER_ADDRESS.trim()));
      console.log("=== FIN DEBUG ===");

      // ── Cortamos acá hasta confirmar que los logs están bien ──
      // Cuando todo muestre "true" en isAddress, sacás estas dos líneas:
      setIsSigning(false);
      return;
      // ─────────────────────────────────────────────────────────
  ...
  return (
    <PageContainer>
      {/* HEADER — fix: LoginButton es span, no <a>, para evitar <a> dentro de <a> */}
      <HeaderBar>
        <HeaderLeft>
        ...
        <HeaderRight>
          ...
          {/* Fix clave: sin passHref, y LoginButton como span */}
          <Link href="/">
      ...
      <ContentWrapper>
        ...
        {!loading && freelancer && servicio && (
          <>
            <AgreementCard>
              <FreelancerPreview>
                <PreviewAvatar>{getInitials(freelancer.nombre)}</PreviewAvatar>
                <PreviewInfo>
              ...
              <Footer>
                ...
                <FooterValue>BNB Smart Chain Testnet</FooterValue>
              </Footer>
            </AgreementCard>
            ...
            <ActionSection>
              {!address ? (
                ...
                <WalletChip>✓ {address.slice(0, 6)}...{address.slice(-4)}</WalletChip>
              )}
              ...
              <InfoBox>
                ...
                <InfoText>
                  ...
                  bloqueados en el contrato hasta que el trabajo sea aprobado.
                </InfoText>
              </InfoBox>
...
// Fix: span en lugar de a — evita <a> dentro de <a>
